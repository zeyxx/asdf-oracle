...
/**
 * Get all PumpFun token holdings for a wallet
 */
async function getWalletPumpFunTokens(address) {
  log('DEBUG', `[WalletScore] Fetching tokens for ${address.slice(0, 8)}...`);
  ...
  // Filter PumpFun tokens with balance > 0
  const pumpTokens = result.value
    .filter(acc => {
      const info = acc.account?.data?.parsed?.info;
      return info &&
             isPumpFunToken(info.mint) &&
             parseInt(info.tokenAmount?.amount || '0') > 0;
    })
    .map(acc => {
      const info = acc.account.data.parsed.info;
      return {
  ...
  log('DEBUG', `[WalletScore] Found ${pumpTokens.length} PumpFun tokens`);
  return pumpTokens;
}

// Cache for wallet transfer history (avoid refetching for each token)
const transferCache = new Map();
const TRANSFER_CACHE_TTL = 5 * 60 * 1000; // 5 minutes

/**
 * Get all token transfers for a wallet (cached)
 * Uses Helius Enhanced API - much faster than RPC
 */
async function getWalletTransfers(wallet) {
  const cached = transferCache.get(wallet);
  if (cached && Date.now() - cached.fetchedAt < TRANSFER_CACHE_TTL) {
    return cached.transfers;
  }

  log('DEBUG', `[WalletScore] Fetching transfers for ${wallet.slice(0, 8)} via Enhanced API...`);

  try {
    const transfers = await helius.getTokenTransfers(wallet, {
      limit: 100,
      maxPages: 5, // Up to 500 transfers
    });

    transferCache.set(wallet, {
      transfers,
      fetchedAt: Date.now(),
    });

    log('DEBUG', `[WalletScore] Got ${transfers.length} transfers for ${wallet.slice(0, 8)}`);
    return transfers;
  } catch (error) {
    log('WARN', `[WalletScore] Enhanced API error: ${error.message}`);
    return [];
  }
}

/**
 * Analyze token activity for a wallet from cached transfers
 * Returns: { hasBuys, hasSells, totalBought, totalSold, netFlow }
 */
function analyzeTokenActivity(wallet, mint, transfers) {
  const tokenTransfers = transfers.filter(t => t.mint === mint);
  ...
  for (const t of tokenTransfers) {
    ...
    if (t.fromUser === wallet) {
      totalSold += t.amount;
    }
  }

  return {
    hasBuys: totalBought > 0,
    hasSells: totalSold > 0,
    totalBought,
    totalSold,
    netFlow: totalBought - totalSold,
    txCount: tokenTransfers.length,
  };
}

/**
 * Get first buy amount for a wallet on a specific token
 * Uses cached transfer history from Enhanced API
 */
async function getFirstBuyAmount(wallet, mint) {
  try {
    const transfers = await getWalletTransfers(wallet);

    // Filter transfers for this mint where wallet received tokens
    const buys = transfers
      .filter(t => t.mint === mint && t.toUser === wallet && t.amount > 0)
    ...
    return {
      amount: firstBuy.amount,
      timestamp: firstBuy.timestamp,
      signature: firstBuy.signature,
    };
  } catch (error) {
    log('WARN', `[WalletScore] Error getting first buy for ${mint.slice(0, 8)}: ${error.message}`);
    return null;
  }
}

/**
 * Calculate retention for a single token
 * Uses activity analysis from Enhanced API
 */
async function calculateTokenRetention(wallet, token) {
  // Optimization: Use local DB for our tracked token
  if (token.mint === TOKEN_MINT) {
    const localData = await db.getWalletKScore(wallet);
    if (localData) {
      return {
        mint: token.mint,
        current_balance: parseInt(localData.current_balance),
        first_buy_amount: parseInt(localData.first_buy_amount),
        retention: localData.retention,
        classification: localData.classification,
        first_buy_ts: localData.first_seen_at,
        data_quality: 'local_db',
      };
    }
  }

  // For other tokens, use Enhanced API transfers
  const transfers = await getWalletTransfers(wallet);
  const activity = analyzeTokenActivity(wallet, token.mint, transfers);

  // If we have activity data, use it to determine behavior
  if (activity) {
    // Has sells = reducer or extractor
    if (activity.hasSells) {
      // netFlow < 0 means sold more than bought recently = extractor
      // netFlow > 0 means bought more than sold = reducer (partial sell)
      const retention = activity.netFlow >= 0 ? 0.7 : 0.3;
      return {
        mint: token.mint,
        current_balance: token.balance,
        first_buy_amount: token.balance, // Unknown
        retention,
        classification: activity.netFlow >= 0 ? 'reducer' : 'extractor',
        first_buy_ts: null,
        data_quality: 'activity_based',
        activity: {
          bought: activity.totalBought,
          sold: activity.totalSold,
          net: activity.netFlow,
          txs: activity.txCount,
        },
      };
    }

    // Has buys but no sells = accumulator or holder
    if (activity.hasBuys && !activity.hasSells) {
      return {
        mint: token.mint,
        current_balance: token.balance,
        first_buy_amount: token.balance,
        retention: 1.2, // Recent accumulation
  ...
  // No recent activity - old holding, assume holder
  return {
    mint: token.mint,
    current_balance: token.balance,
    first_buy_amount: token.balance,
